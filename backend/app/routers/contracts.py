...
def get_my_contracts(
    ...
    # Build job title + org name lookup so UI can show meaningful contract names
    job_ids = {c.job_id for c in contracts if c.job_id}
    org_ids = {c.organization_id for c in contracts}
    ...
    for c in contracts:
        ...
        org = orgs_map.get(c.organization_id)
        item = ContractListOut.model_validate(c).model_dump()
        item['job_title'] = job.title if job else None
        item['org_name'] = org.name if org else None
        item['description'] = job.title if job else None
        out.append(item)
