...
class TesseractEngine(OCREngine):
    ...
    def __init__(self):
        ...
        try:
            ...
            # Configure tesseract binary path + tessdata dir from env
            tesseract_cmd = os.getenv("TESSERACT_CMD")
            if tesseract_cmd and os.path.exists(tesseract_cmd):
                pytesseract.pytesseract.tesseract_cmd = tesseract_cmd
                logger.info(f"  [Tesseract] Using binary: {tesseract_cmd}")
            tessdata_prefix = os.getenv("TESSDATA_PREFIX")
            if tessdata_prefix and os.path.isdir(tessdata_prefix):
                os.environ["TESSDATA_PREFIX"] = tessdata_prefix
                logger.info(f"  [Tesseract] Using TESSDATA_PREFIX: {tessdata_prefix}")
            # Smoke test: verify binary callable + languages available
            try:
                available = pytesseract.get_languages(config="")
                if "vie" not in available and "eng" not in available:
                    logger.warning(
                        f"  [Tesseract] No traineddata found at {os.environ.get('TESSDATA_PREFIX', 'default')}. "
                        f"Available: {available}"
                    )
                else:
                    logger.info(f"  [Tesseract] Languages available: {available}")
            except Exception as e:
                logger.warning(f"  [Tesseract] Language check failed: {e}")
        except ImportError as e:
            logger.warning(f"  [Tesseract] Import failed: {e}. Engine disabled.")
    ...
    def process_image(self, file_path: str) -> OCRResult:
        ...
        try:
            ...
            # Use tempfile để cross-platform (Windows: %TEMP%, Unix: /tmp)
            tmp_dir = tempfile.gettempdir()
            cache_path = os.path.join(tmp_dir, f"_cv_{os.getpid()}.png")
...
def _extract_pdf_scan_pages(file_path: str) -> Tuple[List[Dict[str, Any]], int]:
    ...
    try:
        ...
        for page_num in range(1, total_pages + 1):
            ...
            cache_path = os.path.join(tempfile.gettempdir(), f"_cv_pdfscan_{os.getpid()}_{page_num}.png")
            pix.save(cache_path)
