...
Return raw JSON only. If a section cannot be found in the text, OMIT it from extracted_fields entirely instead of including empty arrays."""
    ...
    # ── 4. Gọi AI ─────────────────────────────────────────────────────────
    #
    # Strategy: thử Vision trước (nếu need_vision), rồi LUÔN thử Text LLM sau.
    # Vision fail → không skip text LLM, vì text LLM vẫn có thể parse
    # partial OCR text hoặc dùng base64 image.
    #
    vision_attempted = False
    text_attempted = False

    for idx, config in enumerate(api_pool, start=1):
        is_vision_config = config.get("is_vision", False)

        # ── Vision config: chỉ thử khi cần vision ────────────────────────
        if is_vision_config:
            if not need_vision:
                logger.info(
        ...
        # ── Text config HOẶC Vision config dùng text mode ────────────────
        # Text LLM luôn thử, kể cả khi vision đã fail
        timeout_sec = config.get("timeout_sec", 45 if is_vision_config else 15)
        logger.info(
            f"  [Normalizer] Thử #{idx}/{len(api_pool)}: {config['name']} "
            f"(is_vision={is_vision_config}, timeout={timeout_sec}s)..."
        )
        ...
        try:
            result = call_llm_api(
                ...
                system_prompt,
                combined_prompt_text,
                is_image=(is_image and is_vision_config),
                b64_url=b64_url if is_vision_config else None,
    ...
    if vision_attempted and not text_attempted:
        logger.info("  [Normalizer] Vision fail, text chưa thử — thử text LLM lần cuối...")
        # Force thử tất cả text LLMs (is_vision=False) một lần nữa
        for idx, config in enumerate(api_pool, start=1):
            if config.get("is_vision", False):
                continue  # skip vision configs
            try:
                result = call_llm_api(
                    config,
                    system_prompt,
                ...
                if result:
                    logger.info(f"  [Normalizer] ✓ Text LLM fallback ({config['name']}) thành công")
                    return result
            except Exception as err:
                logger.warning(f"  [Normalizer] Text LLM fallback #{idx} failed: {err}")
