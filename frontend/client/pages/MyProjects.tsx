...
import { useContract, useMyContracts } from "@/hooks/use-contracts";
...
function ContractCard({ contract }: { contract: ContractListItem }) {
  ...
  const tone = STATUS_TONE[contract.status.toUpperCase()] ?? STATUS_TONE.DRAFT;
  const milestones = detail?.milestones ?? [];
  const completedMilestones = milestones.filter(
    (m) => m.status.toUpperCase() === "APPROVED" || m.status.toUpperCase() === "PAID"
  ).length;
  ...
  return (
    <article
      className="group rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm transition hover:border-indigo-200"
    >
      ...
      <div className="flex items-center justify-between border-t border-slate-100 pt-3">
        ...
        <span className="flex items-center gap-1 text-[10px] font-bold text-slate-500">
          {contract.status === "ACTIVE" ? "Đang thực hiện" : contract.status === "COMPLETED" ? "Hoàn thành" : "Chi tiết"}
...
export default function MyProjects({ contracts, isLoading }: MyProjectsProps) {
  ...
  const { data: contractsData, isLoading: loading } = useMyContracts();
