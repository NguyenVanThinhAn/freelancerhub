...
import { ENDPOINT_DISPUTES, ENDPOINT_DISPUTES_ID, ENDPOINT_DISPUTES_ID_EVIDENCE, ENDPOINT_CONTRACTS_ID_DISPUTES } from "@/api/endpoints";
...
interface Dispute {
  ...
  initiator_id: string;
  reason: string;
  status: "OPEN" | "UNDER_REVIEW" | "RESOLVED_FREELANCER" | "RESOLVED_CLIENT" | "MUTUAL_AGREEMENT";
...
export default function Disputes() {
  ...
  const [createForm, setCreateForm] = useState({ contract_id: "", milestone_id: "", reason: "", severity: "medium" });
  ...
  const { data: evidences } = useQuery({
    ...
    queryFn: () => apiGet<DisputeEvidence[]>(`/disputes/${disputeId}/evidence`).catch(() => []),
    enabled: !!disputeId,
    staleTime: 30_000,
  });

  const createDispute = useMutation({
    mutationFn: (payload: typeof createForm) => apiPost<Dispute>(ENDPOINT_DISPUTES, payload),
    onSuccess: () => {
    ...
    return (
      <BusinessShell active="Hợp đồng">
        ...
        <section className="mb-5 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-[10px] text-slate-400">
                Hợp đồng #{dispute.contract_id.slice(0, 8)} · {new Date(dispute.created_at).toLocaleString("vi-VN")}
              </p>
            </div>
            <span className={`rounded-full px-3 py-1.5 text-[10px] font-bold ${STATUS_TONE[dispute.status] ?? "bg-slate-100"}`}>
              {STATUS_LABEL[dispute.status] ?? dispute.status}
            </span>
          </div>
          <div className="mt-4 rounded-lg border border-amber-100 bg-amber-50/50 p-3">
            <p className="text-[10px] font-bold text-amber-700">Lý do mở dispute:</p>
            <p className="mt-1 whitespace-pre-line text-[10px] text-slate-700">{dispute.reason}</p>
          </div>
          {dispute.resolution_notes && (
            <div className="mt-3 rounded-lg border border-emerald-100 bg-emerald-50/50 p-3">
              ...
              <p className="mt-1 whitespace-pre-line text-[10px] text-slate-700">{dispute.resolution_notes}</p>
            </div>
          )}
        </section>
        ...
        <section className="mb-5 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          ...
          >
            {submitEvidence.isPending ? <Loader2 size={12} className="animate-spin" /> : <Send size={12} />}
            Gửi minh chứng
          </button>
        </section>
        ...
        <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          <h2 className="mb-3 text-xs font-extrabold">Lịch sử minh chứng ({evidences?.length ?? 0})</h2>
          {!evidences || evidences.length === 0 ? (
            <p className="text-center text-[10px] text-slate-400 py-6">Chưa có minh chứng nào.</p>
          ) : (
              ...
              {evidences.map((ev) => (
                <div key={ev.id} className="rounded-lg border border-slate-100 p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <p className="text-[9px] text-slate-400">{new Date(ev.submitted_at).toLocaleString("vi-VN")}</p>
                  </div>
                  <p className="mt-2 whitespace-pre-line text-[10px] text-slate-600">{ev.evidence_text}</p>
                  {ev.file_urls && ev.file_urls.length > 0 && (
                    <div className="mt-2 flex gap-2">
                      {ev.file_urls.map((url, i) => (
                        <a key={i} href={url} target="_blank" rel="noreferrer" className="flex items-center gap-1 rounded bg-slate-50 px-2 py-1 text-[9px] text-indigo-600 hover:bg-slate-100">
                          <FileText size={10} />
                          File {i + 1}
                        </a>
                      ))}
    ...
    <BusinessShell active="Hợp đồng">
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">Mở và theo dõi các dispute với hợp đồng của bạn.</p>
        </div>
        <button
          type="button"
        ...
        {listLoading ? (
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-slate-400 mt-1">
              Để xem dispute, mở từ trang chi tiết hợp đồng hoặc tạo mới ở nút trên.
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody>
                {disputes.map((d) => (
                  <tr key={d.id} className="border-b border-slate-50 text-[10px] last:border-0 cursor-pointer hover:bg-slate-50" onClick={() => navigate(`/disputes/${d.id}`)}>
                    <td className="py-3 font-mono">{d.id.slice(0, 8)}</td>
                    <td>{d.contract_id.slice(0, 8)}…</td>
                    <td className="max-w-[300px] truncate">{d.reason}</td>
                    <td>
                      <span className={`rounded-full px-2 py-1 text-[8px] font-semibold ${STATUS_TONE[d.status] ?? "bg-slate-100"}`}>
                        {STATUS_LABEL[d.status] ?? d.status}
                      </span>
                    </td>
                    <td className="text-slate-400">{new Date(d.created_at).toLocaleDateString("vi-VN")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="space-y-3">
              <div>
                <label className="mb-1 block text-[10px] font-bold text-slate-700">Contract ID</label>
                <input
                  value={createForm.contract_id}
                  onChange={(e) => setCreateForm((f) => ({ ...f, contract_id: e.target.value }))}
              ...
              <div>
                <label className="mb-1 block text-[10px] font-bold text-slate-700">Milestone ID (tùy chọn)</label>
                <input
              ...
              <div>
                <label className="mb-1 block text-[10px] font-bold text-slate-700">Mức độ nghiêm trọng</label>
                <select
              ...
              <div>
                <label className="mb-1 block text-[10px] font-bold text-slate-700">Lý do / Mô tả</label>
                <textarea
