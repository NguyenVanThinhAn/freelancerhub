...
> **Phạm vi:** Toàn bộ frontend (`frontend/client/`): 15 file page (`pages/*.tsx` × 15, trong đó `NotFound.tsx` gắn với route catch-all `*`) — 13 path mount riêng biệt + 1 catch-all = 14 `<Route>` trong `App.tsx`; 71 endpoint của backend; theo Sprint 1–4.
...
- **14 file page** (`pages/*.tsx`) gồm 13 page chính + 1 `NotFound.tsx` (catch-all). 13 page chính đều dùng **mock array hardcode** (xem `pages/JobsList.tsx:5-14`, `pages/Matching.tsx:5-13`, `pages/Wallet.tsx:6-7`).
...
> - Frontend **ĐÃ KẾT NỐI**: JobsList ✅, Wallet ✅, Settings ✅, Index ✅, CreateJob ✅, Matching ✅, ContractMilestone ✅. Còn lại: CandidateDetail, ContentInput, ContentResult, AIProcessing, GeneratedJD, InterviewScheduler.
...
| 14 | **Wallet** `/wallet` | Số dư ví | GET | `/wallet` | ✅ | `24,750,000 ₫` hardcode (line 42) |
| 15 | **Wallet** | Lịch sử giao dịch | GET | `/wallet/transactions` | ✅ | `const transactions` (line 7) |
| 16 | **Wallet** | Nạp tiền | POST | `/wallet/deposit` | ✅ | `openDeposit` chỉ navigate |
| 17 | **Wallet** | Rút tiền | POST | `/wallet/withdraw` | ✅ | `openWithdraw` chỉ navigate |
| 18 | **Wallet** | Danh sách dự án ký quỹ | GET | `/contracts/my` (filter status) | ✅ | `const projects` (line 6) |
...
| 56 | | Submit milestone | POST | `/milestones/{id}/submit` | ✅ |
| 57 | | Approve deliverable | POST | `/deliverables/{id}/approve` | ✅ |
| 58 | | Reject deliverable | POST | `/deliverables/{id}/reject` | ✅ |
| 59 | | Complete contract | POST | `/contracts/{id}/complete` | ✅ |
| 60 | **Disputes (new)** | Mở dispute | POST | `/disputes` | ✅ |
| 61 | | Chi tiết dispute | GET | `/disputes/{id}` | ✅ |
| 62 | | Disputes của contract | GET | `/contracts/{id}/disputes` | ✅ |
| 63 | | Thêm evidence | POST | `/disputes/{id}/evidence` | ✅ |
| 64 | | Resolve | POST | `/disputes/{id}/resolve` | ✅ |
| 65 | **Admin (new)** | Danh sách verifications | GET | `/admin/verifications` | ✅ admin |
| 66 | | Chi tiết case | GET | `/admin/verifications/{case_id}` | ✅ admin |
| 67 | | Admin decision | PATCH | `/admin/verifications/{case_id}/decision` | ✅ admin |
| 68 | **Admin** | Users list | GET | `/admin/users` | ✅ admin |
| 69 | | Lock/unlock | PATCH | `/admin/users/{id}/lock` + `/unlock` | ✅ admin |
| 70 | **CreateJob** | Categories | GET/POST | `/categories` | ✅ |
| 71 | **Messages (new)** | Chat threads | GET/POST | `/chat/threads` + `/chat/threads/{id}/messages` | ✅ |
