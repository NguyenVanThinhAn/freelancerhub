...
export async function apiFetch<T>(
  ...
  init: RequestInit & {
    auth?: boolean;
    /**
     * When true, treat HTTP 404 as success with `data: null` instead of throwing.
     * Useful for polling endpoints where 404 means "not ready yet, keep polling".
     */
    acceptNotFound?: boolean;
  } = {},
): Promise<T> {
  const { auth = true, acceptNotFound = false, headers, ...rest } = init;
  ...
  // Special-case: 404 with acceptNotFound = treat as "not ready" polling result
  if (res.status === 404 && acceptNotFound) {
    debugLog("· 404 (not-found, polled)", "#94a3b8", path);
    return null as T;
  }
